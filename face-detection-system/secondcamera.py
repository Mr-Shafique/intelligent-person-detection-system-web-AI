            ...
            if webcam_frame_counter % PROCESS_EVERY_N_FRAMES == 0:
                ...
                for det_idx, det_data in enumerate(current_detections_data):
                    ...
                    if best_match_id != -1:
                        ...
                        # Re-recognition attempt for pending tracks
                            ...
                            if current_roi_for_rerecog is not None and current_roi_for_rerecog.size > 0:
                                ...
                                if live_emb_rerecog:
                                    ...
                                    if cmsId_r: 
                                        ...
                                        # print(f"Webcam: Pending track ID {best_match_id} re-attempt failed. Attempts left: {track['recognition_attempts_left']}. Dist: {dist_r:.3f}")
                                ...
                                else: # Embedding creation failed
                                    track['recognition_attempts_left'] -= 1
                                    if track['recognition_attempts_left'] <= 0:
                ...
                for face_id, track_data in tracked_faces.items():
                    if face_id not in matched_track_ids_this_cycle:
                        if (processing_frame_count_webcam - track_data['last_seen_frame_count']) > MAX_FRAMES_UNSEEN_PROCESSING_CYCLES:
                            ...
                            # Add to recently_lost only if finalized (not pending) AND has an embedding
                ...
                for new_det_data in current_detections_data:
                    ...
                    # Defaults for a new track that might become pending
                    ...
                    current_track_name_on_creation = "Pending" # Display name if it becomes pending
                    ...
                    if live_emb:
                        ...
                        if revived_id:
                            ...
                            is_pending = False
                            attempts_left = 0
                            is_recognized_session = True # Revived from cache
                            print(f"Webcam: Re-ID {initial_name} from lost cache. Dist: {initial_dist:.3f}")
                            del recently_lost_faces_webcam[revived_id]
                        else: # Not in lost cache, try full DB for the *first time*
                            ...
                            if initial_cmsId: # Initial recognition successful
                                ...
                                # print(f"Webcam: New track immediately recognized as {initial_name}. Dist: {initial_dist:.3f}")
                            # else: it remains pending, current_track_name_on_creation is "Pending"
                    ...
                    if is_pending:
                        ...
                        # print(f"Webcam: New track ID {new_id} is PENDING recognition. Initial attempt dist: {initial_dist:.3f if live_emb else 'N/A'}")
                    elif not revived_id: # Recognized on first try, not from cache
            ...
            if cam2_frame_counter % PROCESS_EVERY_N_FRAMES == 0: 
                ...
                for det_idx, det_data in enumerate(current_detections_data_cam2): 
                    ...
                    if best_match_id != -1:
                        ...
                        # Re-recognition attempt for pending tracks on Camera 2
                        if track.get('pending_recognition', False) and track.get('recognition_attempts_left', 0) > 0:
                            current_roi_for_rerecog = det_data.get('face_roi_original')
                            if current_roi_for_rerecog is not None and current_roi_for_rerecog.size > 0:
                                live_emb_rerecog = create_face_embedding_live(current_roi_for_rerecog)
                                if live_emb_rerecog:
                                    ...
                                    if cmsId_r:
                                        ...
                                        track['recognition_attempts_left'] -= 1
                                        # print(f"Camera2: Pending track ID {best_match_id} re-attempt failed. Attempts left: {track['recognition_attempts_left']}. Dist: {dist_r:.3f}")
                                        if track['recognition_attempts_left'] <= 0:
                                            track['pending_recognition'] = False
                                            track['name'] = "Unknown" # Finalize as Unknown
                ...
                for new_det_data in current_detections_data_cam2: 
                    ...
                    if live_emb:
                        ...
                        if revived_id_cam2: 
                            ...
                            attempts_left = 0
                            is_recognized_session = True
                            print(f"Camera2: Re-ID {initial_name} from lost cache. Dist: {initial_dist:.3f}")
                            ...
                            if initial_cmsId:
                                ...
                                attempts_left = 0
                                is_recognized_session = True
                                # print(f"Camera2: New track immediately recognized as {initial_name}. Dist: {initial_dist:.3f}")
                    ...
                    elif not revived_id_cam2: # Recognized on first try, not from cache
